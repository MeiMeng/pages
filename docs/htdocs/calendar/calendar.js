...
var holiday = new Array("2016-12-31","2017-1-1","2017-1-2","2017-1-27","2017-1-28","2017-1-29","2017-1-30","2017-1-31","2017-2-1","2017-2-2","2017-4-2","2017-4-3","2017-4-4","2017-4-29","2017-4-30","2017-5-1","2017-5-28","2017-5-29","2017-5-30","2017-10-1","2017-10-2","2017-10-3","2017-10-4","2017-10-5","2017-10-6","2017-10-7","2017-10-8");

var workday = new Array("2017-1-22","2017-2-4","2017-4-1","2017-5-27","2017-9-30");
...
$(function(){
    ...
    //渲染样式
    $("#headRow button").addClass("btn-sm");            //头部按钮大小
    $("#headRow select").addClass("input-sm");          //头部下拉框大小
    $("."+NYear+"-"+NMonth+"-"+NDate).addClass("today current"); //今天背景色

    //$("."+NYear+"-"+NMonth+"-"+NDate).find(".tip").show(); //显示今天天气提示
    $(".lunarText").hide(); //tip背面文字隐藏

    //调整高度
    var parentH = $("#calendar").parent().height();
    var parentW = $("#calendar").parent().width();
    var cH = $("#calendar").height();
    if(parentW>580 && parentH>650){
        $("#calendar").css("margin-top",(parentH-cH)/4);
    }
...
function __getLastDate(_myDate){
    ...
    console.log("这个月1日"+tempDate.getFullYear()+"-"+(tempDate.getMonth()+1)+"-"+tempDate.getDate());
    ...
    console.log("下个月1日"+_myDate.getFullYear()+"-"+(_myDate.getMonth()+1)+"-"+_myDate.getDate());
    _myDate.setTime(_myDate.getTime()-86400000);
    lastDate = _myDate.getDate();
    lastDayWeek = _myDate.getDay();  //本月最后一天星期几
    console.log("本月最后一天："+_myDate.getFullYear()+"-"+(_myDate.getMonth()+1)+"-"+lastDate+"，星期"+lastDayWeek);
}
    ...
    for(var i=0; i<rows; i++){
        ...
        for(var j=1; j<=7; j++){
            ...
            if(holiday.contains(yearNum+'-'+(parseInt(monthNum)+1)+'-'+dayNum)){
                dateCell += '<div class="holiday">假</div>';
            }
    ...
    $.ajax({
        ...
        success: function(msg){
            ...
            for(var i=0; i<w_days; i++){
                ...
                /*console.log(weatherArr[i].date);
                console.log(num+","+weatherArr[i].cond.txt_d);
                console.log(weatherArr[i].tmp.min+"~"+weatherArr[i].tmp.max);*/
                //天气图标
                ...
                $("."+wYear+"-"+wMonth+"-"+wDate+" .weather").empty().append('<img class="weatherImg"  src="http://files.heweather.com/cond_icon/'+num+'.png">');
